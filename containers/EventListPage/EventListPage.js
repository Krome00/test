...
class EventListPage extends Component {
	...
	renderTable() {
		const { events } = this.props.eventListPageProps || {};
		let data = [];

		events && events.length && events.map( event => {
			const { _id: id, user_id, location_id, door_id, time, event_type, result } = event;
			data.push({
				id,
				name: user_id.name,
				...
				event_type: LabelHelper.event_labels(event_type),
				result: LabelHelper.event_labels(result),
				key: id
			});
		});
	...
	render() {
		const { loading } = this.props.eventListPageProps;
		return (
